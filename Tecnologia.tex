...
En el Cuadro~\ref{tab:comparativa-backend} se resumen las características fundamentales de cada entorno.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{1.9cm} >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X}
        \textbf{Criterio} & \textbf{Spring Boot (Kotlin)} & \textbf{NestJS (TypeScript)} & \textbf{Ktor (Kotlin)} \\ \lightrule
        \textbf{Paradigma} & Multihilo / Síncrono & Basado en eventos & Asíncrono (Corrutinas) \\ \lightrule
        \textbf{Seguridad de tipos} & Estricta (Nativa) & Alta (Transpilado) & Estricta (Nativa) \\ \lightrule
        \textbf{Ecosistema} & Muy maduro & Masivo (NPM) & En crecimiento \\ \lightrule
        \textbf{Arquitectura} & Altamente estructurada & Modular & Minimalista \\ \lightrule
    \end{tabularx}
...
En el Cuadro~\ref{tab:comparativa-bd} se resumen las principales características técnicas evaluadas.

\begin{table}[H]
    \renewcommand{\arraystretch}{1.25}
    \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{1.9cm} >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X}
        \textbf{Criterio} & \textbf{PostgreSQL} & \textbf{MySQL (InnoDB)} & \textbf{MariaDB} \\ \lightrule
        \textbf{Modelo} & Objeto-relacional & Relacional & Relacional \\ \lightrule
        \textbf{Cumplimiento SQL} & Estricto & Medio-alto & Medio-alto \\ \lightrule
        \textbf{Consultas complejas} & Excelente (CTE, sub consultas) & Adecuado & Bueno \\ \lightrule
        \textbf{Tipos avanzados} & Arrays, \texttt{JSONB} indexable, UUID & JSON básico & JSON básico \\ \lightrule
    \end{tabularx}
...
En el Cuadro~\ref{tab:comparativa-os} se analizan las soluciones basándonos en las necesidades del entorno de desarrollo y producción del proyecto.

\begin{table}[H]
    \renewcommand{\arraystretch}{1.25}
    \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{1.9cm} >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X}
        \textbf{Criterio} & \textbf{MinIO} & \textbf{Amazon S3} & \textbf{Firebase Storage} \\ \lightrule
        \textbf{Despliegue} & Local / Autoalojado (Docker) & Nube gestionada (AWS) & Nube gestionada (GCP) \\ \lightrule
        \textbf{Compatibilidad API} & Estándar S3 & Estándar S3 & Propietaria \\ \lightrule
        \textbf{Soberanía de datos} & Total (Bajo control propio) & Cedida al proveedor & Cedida al proveedor \\ \lightrule
        \textbf{Gestión de Infraestructura} & Responsabilidad del desarrollador & Automática & Automática \\ \lightrule
    \end{tabularx}
...
\section{Aplicación \emph{front-end}}
...
En el Cuadro~\ref{tab:comparativa-frontend} se detalla la comparativa técnica de las tecnologías evaluadas.

\begin{table}[H]
    \renewcommand{\arraystretch}{1.25}
    \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{1.9cm} >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}X}
        \textbf{Criterio} & \textbf{React Native + Expo} & \textbf{Flutter} & \textbf{Ionic} \\ \lightrule
        \textbf{Renderizado UI} & Componentes Nativos & Motor gráfico propio & WebView \\ \lightrule
        \textbf{Lenguaje} & TypeScript / JavaScript & Dart & TypeScript / Web \\ \lightrule
        \textbf{Acceso a Hardware} & Mediante puente (JSI) & Integración compilada & Plug-ins (Capacitor) \\ \lightrule
        \textbf{Desarrollo ágil} & Muy alto (Expo) & Alto & Muy alto \\ \lightrule
    \end{tabularx}
